        ...
        if(body != nullptr && aCol != nullptr && body->BodyType() == BodyType::dynamicBody) { //Checks from dynamicBody perspective
            ...
            for(auto& receiver : collidableObjects) { //Currently only boxCollision
                if(receiver != initiator) {
                    ...
                    auto collision = CheckBoxCollision(initiator->GetTransform().position, *aCol, receiver->GetTransform().position, *bCol);
...
void PhysicsSystem::RemainCollision(const shared_ptr<GameObject>& initiator, const shared_ptr<Collider>& init_collider,
    ...
    init_collision.Contacts(std::get<0>(direction));
    auto rec_collision = rec_collider->GetCollisionById(collisionId);
    rec_collision.Contacts(std::get<1>(direction));
...
auto PhysicsSystem::CheckBoxCollision(Point aPos, const BoxCollider& aCol, Point bPos, const BoxCollider& bCol) -> std::unique_ptr<std::tuple<std::vector<CollisionPoint>, std::vector<CollisionPoint>>> {
    ...
    if(x_overlap && y_overlap) {
        ...
        auto a_collisions = std::vector<CollisionPoint>();
        auto b_collisions = std::vector<CollisionPoint>();

        if (top_col < bottom_col && top_col < left_col && top_col < right_col ){//Bottom collision
            a_collisions.push_back(CollisionPoint::Bottom);
            b_collisions.push_back(CollisionPoint::Top);
        ...
        if (bottom_col < top_col && bottom_col < left_col && bottom_col < right_col){ //Top collision
            a_collisions.push_back(CollisionPoint::Top);
            b_collisions.push_back(CollisionPoint::Bottom);
        }
        if (left_col < right_col && left_col < top_col && left_col < bottom_col) { //Right collision
            a_collisions.push_back(CollisionPoint::Right);
            b_collisions.push_back(CollisionPoint::Left);
        }
        if (right_col < left_col && right_col < top_col && right_col < bottom_col ) { //Left collision
            a_collisions.push_back(CollisionPoint::Left);
            b_collisions.push_back(CollisionPoint::Right);
        }

        return std::make_unique<std::tuple<std::vector<CollisionPoint>, std::vector<CollisionPoint>>>(std::make_tuple(a_collisions, b_collisions));
    }
