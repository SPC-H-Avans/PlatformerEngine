...
private:
     ...
     */
    void CreateCollision(std::shared_ptr<GameObject> initiator, std::shared_ptr<Collider> init_collider, std::shared_ptr<GameObject> receiver, std::shared_ptr<Collider> rec_collider);
    ...
    std::map<std::shared_ptr<Collider>, std::vector<std::shared_ptr<Collider>>> collisions;
};
